...
$outpath = '/nfs/sc/disks/sdg74_3328/PycharmProjects/spr_qismat/stats_20ww24_in_progress/sch';
...
print STDERR "parsing sch list file...\n";
...
for $href( @blocklist ){
    ...
    for $schref( @schlist ) {
       ...
       #if( $schref->{'name'} eq $href->{'template'}.".apr_final.vg.gz" ) {
       if( $schref->{'name'} eq $href->{'template'}.".vg.gz" ) {
           $found=1;
